...
int InstallWireGuard(){
    ...
    system("sysctl -w net.ipv6.conf.default.forwarding=1");
    ...
    fprintf(server_config, "Address = 10.0.0.1/32, fd00:7::1/48\n");//服务器ip地址，修改需要同时修改客户端配置
...
int AddUser() {
    ...
    system(command);
    server_config = fopen("/etc/wireguard/wg0.conf", "a");
    fprintf(server_config, "AllowedIPs = 10.0.0.%d/32, fd00:7::%d/48\n",num,num);//客户端ip地址分配，不要修改
    ...
    fprintf(client_config, "Address = 10.0.0.%d/32, fd00:7::%d/48\n",num,num);
