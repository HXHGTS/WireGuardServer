...
int InstallWireGuard(){
    ...
    fprintf(server_config, "PostUp = iptables -A FORWARD -i wg0 -j ACCEPT; iptables -A FORWARD -o wg0 -j ACCEPT;iptables -t nat -A POSTROUTING -o eth0 -j FULLCONENAT;iptables -t nat -A PREROUTING -i eth0 -j FULLCONENAT;service iptables save\n");//服务器防火墙配置
    fprintf(server_config, "PostDown = iptables -D FORWARD -i wg0 -j ACCEPT; iptables -D FORWARD -o wg0 -j ACCEPT;iptables -t nat -D POSTROUTING -o eth0 -j FULLCONENAT;iptables -t nat -D PREROUTING -i eth0 -j FULLCONENAT;service iptables save\n");//服务器防火墙配置
